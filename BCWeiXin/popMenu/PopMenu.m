...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews){
        if ([view isKindOfClass:[PopMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]){
...
@implementation PopMenuView{
    PopMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    PopMenuSelectedItem _selectedItem;
}
...
- (id)init{
    ...
    if(self){
        ...
        if([PopMenu hasShadow]){
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [PopMenu tintColor].CGColor;
...
-(void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect{
    ...
    if (heightPlusArrow <(outerHeight - rectY1)){
        ...
        _arrowDirection = PopMenuViewArrowDirectionUp;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,rectY1
        };
        ...
        _arrowDirection = PopMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM -widthHalf,
        ...
        _arrowDirection = PopMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = PopMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = PopMenuViewArrowDirectionNone;
        self.frame = (CGRect){
            (outerWidth - contentSize.width)*0.5,
            (outerHeight - contentSize.height)*0.5,
            contentSize,
        };
    ...
    for(PopMenuItem *menuItem in _menuItems){
        ...
        if(menuItem.title.length){
            ...
            if(!menuItem.enabled && !menuItem.image){
                titleFrame = (CGRect){
                    kMarginX*2,kMarginY,maxItemWidth - kMarginX * 4,maxItemHeight - kMarginY *2
                };

            }else{
                titleFrame = (CGRect){
                    titleX,kMarginY,titleWidth,maxItemHeight - kMarginY*2
                };
            }
        ...
        if(menuItem.image){
            const CGRect imageFrame = {
                kMarginX*2,kMarginY,maxItemWidth,maxItemHeight-kMarginY*2};
            UIImageView *imageView = [[UIImageView alloc]initWithFrame:imageFrame];
            imageView.image = menuItem.image;
            imageView.clipsToBounds = YES;
            imageView.contentMode = UIStackViewAlignmentCenter;
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == PopMenuViewArrowDirectionUp) {

        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };

    } else if (_arrowDirection == PopMenuViewArrowDirectionDown) {

        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };

    } else if (_arrowDirection == PopMenuViewArrowDirectionLeft) {

        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };

    } else if (_arrowDirection == PopMenuViewArrowDirectionRight) {
...
+ (UIImage *)selectedImage:(CGSize)size{
    ...
    const CGFloat components[] = {
        0.216,0.471,0.871,1,0.059,0.353,0.839,1,
    };
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    if ([PopMenu backgroundColorEffect] == PopMenuBackGroundColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == PopMenuViewArrowDirectionLeft ||
        _arrowDirection == PopMenuViewArrowDirectionRight) {
...
/// 背景色效果
static PopMenuBackGroundColorEffect   gBackgroundColorEffect = PopMenuBackGroundColorEffectSolid;
/// 是否显示阴影
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = PopMenuBackGroundColorEffectSolid;
    gHasShadow = NO;
}
